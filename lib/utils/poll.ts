...
export function validatePollOptions(options: string[]): string[] {
  ...
  const uniqueOptions = Array.from(new Set(cleanedOptions.map(opt => opt.toLowerCase()))).map(lowerCaseOpt =>
...
export function getPollAnalytics(poll: Poll) {
  ...
  const mostPopular = options.length > 0 
    ? options.reduce((prev, current) => 
        (current.vote_count || 0) > (prev.vote_count || 0) ? current : prev
      )
    : null;

  const averageVotesPerOption = options.length > 0 ? totalVotes / options.length : 0;
